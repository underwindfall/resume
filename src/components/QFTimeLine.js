...
const Item = ({
  ...
  return (
    <GridContainer justify="center" alignItems="center" spacing={3} className={classes.workDetails}>
      <QFGridItem xs={12} sm={3} md={2} className={classes.workYear}>
        <QFText style={{ ...styles.prevY, ...styles.span }} text={startTime} font="bold" />
        <QFText style={{ ...styles.afterY, ...styles.span }} text={endTime} font="bold" />
      </QFGridItem>
      <QFGridItem xs={12} sm={9} md={10}>
        ...
        <div style={styles.exCon}>
          <QFText text={companyName} variant="h5" className={classes.company} />
          <QFText text={title} variant="subtitle1" className={classes.title} />
...
export const QF = ({ items = {}, ...remainProps }) => {
  const classes = useStyles();
  return (
    <PageContainer className={classes.container} {...remainProps}>
      {items.map((item, index) => (
        <Item key={index} classes={classes} {...item} />
      ))}
    </PageContainer>
  );
};
