...
export const QFAppBar = ({ shouldChangeColor = false, showAvatarLogo = false, ...remainProps }) => {
  const classes = useStyles();
  const [mobileOpen, setMobileOpen] = useState(false);

  const handleDrawerToggle = () => setMobileOpen(prevMobileOpen => !prevMobileOpen);
      ...
      <Toolbar className={classes.container}>
        <div className={classes.flex}>
          {showAvatarLogo ? (
            ...
            <QFText text={strings.header.title} className={classes.title} font="bold" />
          )}
        </div>
        <Hidden smDown>
