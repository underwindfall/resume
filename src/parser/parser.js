...
export const parseExperiences = data => {
  ...
  const experiences =
    response.layer.work && response.layer.work.length > 0
      ? response.layer.work.map(experience => parseExperience(response.content[experience]))
      : response.layer.work;
  return {
    error,
    response: {
...
export const parseEducations = data => {
  ...
  const educations =
    response.layer.education && response.layer.education.length > 0
      ? response.layer.education.map(experience => parseEducation(response.content[experience]))
...
export const parseProjects = data => {
  ...
  const projects =
    response.layer.project && response.layer.project.length > 0
      ? response.layer.project.map(experience => parseProject(response.content[experience]))
...
export const parseBlogData = data => keyword => genericParse => {
  return data.layer[keyword] && data.layer[keyword].length > 0
    ? data.layer[keyword].map(experience => genericParse(data.content[experience]))
    : data.layer[keyword];
};
